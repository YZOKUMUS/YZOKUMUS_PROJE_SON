...
/**
 * Hız Modu (Time Attack) - TEST
 * 10 saniyelik zamanlayıcı ile kelime çevirme
 */
let hizModuTimer = null;
let hizModuTimeLeft = 10;

async function startHizModuGame() {
    const data = await loadKelimeData();
    if (data.length === 0) {
        showToast('Kelime verisi yüklenemedi', 'error');
        goToMainMenu();
    ...
    document.getElementById('hiz-modu-screen').classList.remove('hidden');
    document.getElementById('hiz-total-questions').textContent = CONFIG.QUESTIONS_PER_GAME;
    
    loadHizModuQuestion();
}

function loadHizModuQuestion() {
    if (questionIndex >= currentQuestions.length) {
        clearInterval(hizModuTimer);
        endGame();
        return;
    }
    ...
    // Reset timer
    hizModuTimeLeft = 10;
    document.getElementById('hiz-timer').textContent = hizModuTimeLeft;
    ...
    document.getElementById('hiz-question-number').textContent = questionIndex + 1;
    document.getElementById('hiz-combo').textContent = comboCount;
    document.getElementById('hiz-session-score').textContent = formatNumber(sessionScore);
    
    // Display Arabic word
    document.getElementById('hiz-arabic').textContent = currentQuestion.kelime || currentQuestion.arabic || '';
    ...
    const optionsContainer = document.getElementById('hiz-options');
    optionsContainer.innerHTML = options.map((option, index) => `
        <button class="answer-option" onclick="checkHizModuAnswer(${index}, '${option.replace(/'/g, "\\'")}')">
            ${option}
        </button>
    `).join('');
    
    // Start timer
    hizModuTimer = setInterval(() => {
        hizModuTimeLeft--;
        document.getElementById('hiz-timer').textContent = hizModuTimeLeft;
        
        if (hizModuTimeLeft <= 0) {
            clearInterval(hizModuTimer);
            // Time's up - wrong answer
            checkHizModuAnswer(-1, '');
        } else if (hizModuTimeLeft <= 3) {
            document.getElementById('hiz-timer').style.color = '#ff0000';
        }
    }, 1000);
...
function checkHizModuAnswer(index, selectedAnswer) {
    clearInterval(hizModuTimer);
    stopAllAudio();
    
    const correctAnswer = currentQuestion.anlam || currentQuestion.translation;
    const wordId = currentQuestion.kelime_id || currentQuestion.id;
    const buttons = document.querySelectorAll('#hiz-options .answer-option');
    ...
    const isCorrect = index >= 0 && selectedAnswer === correctAnswer;
    ...
    if (isCorrect) {
        ...
        const timeBonus = Math.floor(hizModuTimeLeft * 2);
        const basePoints = getBasePoints(currentDifficulty);
        const gained = basePoints + CONFIG.COMBO_BONUS_PER_CORRECT + timeBonus;
        sessionScore += gained;
        
        showToast(`✅ Doğru! +${gained} Hasene (Hız bonusu: +${timeBonus})`, 'success', 1500);
    } else {
        wrongCount++;
        comboCount = 0;
        ...
        if (index >= 0) {
            ...
            showToast('⏱️ Süre doldu!', 'warning', 1500);
        }
    }
...
/**
 * Eşleştirme Modu - TEST
 * Kart eşleştirme oyunu
 */
let eslestirmeCards = [];
let eslestirmeSelectedCards = [];
let eslestirmePairsFound = 0;
let eslestirmeStartTime = null;
let eslestirmeTimer = null;

async function startEslestirmeGame() {
    const data = await loadKelimeData();
    if (data.length === 0) {
        showToast('Kelime verisi yüklenemedi', 'error');
        goToMainMenu();
    ...
    const selectedWords = getRandomItems(filtered, 6);
    
    // Create pairs: Arabic word + Turkish meaning
    eslestirmeCards = [];
    selectedWords.forEach((word, index) => {
        eslestirmeCards.push({
            id: `card-${index}-ar`,
            type: 'arabic',
            word: word.kelime || word.arabic || '',
            pairId: index,
            matched: false
        });
        eslestirmeCards.push({
            id: `card-${index}-tr`,
            type: 'turkish',
            word: word.anlam || word.translation || '',
            pairId: index,
            matched: false
        });
    });
    ...
    // Shuffle cards
    eslestirmeCards = shuffleArray(eslestirmeCards);
    
    eslestirmeSelectedCards = [];
    eslestirmePairsFound = 0;
    sessionScore = 0;
    eslestirmeStartTime = Date.now();
    
    document.getElementById('eslestirme-screen').classList.remove('hidden');
    document.getElementById('eslestirme-total-pairs').textContent = selectedWords.length;
    
    renderEslestirmeCards();
    
    // Start timer
    eslestirmeTimer = setInterval(() => {
        const elapsed = Math.floor((Date.now() - eslestirmeStartTime) / 1000);
        document.getElementById('eslestirme-time').textContent = elapsed;
    }, 1000);
}

function renderEslestirmeCards() {
    const grid = document.getElementById('eslestirme-cards-grid');
    grid.innerHTML = eslestirmeCards.map(card => `
        <div class="matching-card ${card.matched ? 'matched' : ''} ${eslestirmeSelectedCards.includes(card.id) ? 'selected' : ''}" 
             data-card-id="${card.id}" 
             data-pair-id="${card.pairId}"
             onclick="handleEslestirmeCardClick('${card.id}')">
            <div class="card-content">
                ${card.matched ? '✓' : (eslestirmeSelectedCards.includes(card.id) ? card.word : '?')}
            </div>
        </div>
    `).join('');
    
    // Add CSS if not exists
    if (!document.getElementById('matching-cards-style')) {
        const style = document.createElement('style');
        style.id = 'matching-cards-style';
        style.textContent = `
...
function handleEslestirmeCardClick(cardId) {
    const card = eslestirmeCards.find(c => c.id === cardId);
    if (!card || card.matched || eslestirmeSelectedCards.includes(cardId)) {
        return;
    }
    
    if (eslestirmeSelectedCards.length >= 2) {
        // Reset selection
        eslestirmeSelectedCards = [];
        renderEslestirmeCards();
    }
    
    eslestirmeSelectedCards.push(cardId);
    renderEslestirmeCards();
    
    if (eslestirmeSelectedCards.length === 2) {
        // Check if match
        const card1 = eslestirmeCards.find(c => c.id === eslestirmeSelectedCards[0]);
        const card2 = eslestirmeCards.find(c => c.id === eslestirmeSelectedCards[1]);
        
        if (card1 && card2 && card1.pairId === card2.pairId && card1.type !== card2.type) {
            // Match!
            card1.matched = true;
            card2.matched = true;
            eslestirmePairsFound++;
            
            const basePoints = getBasePoints(currentDifficulty);
            const timeBonus = Math.max(0, 30 - Math.floor((Date.now() - eslestirmeStartTime) / 1000));
            const gained = basePoints + timeBonus;
            sessionScore += gained;
            
            document.getElementById('eslestirme-pairs-found').textContent = eslestirmePairsFound;
            document.getElementById('eslestirme-session-score').textContent = formatNumber(sessionScore);
            
            showToast(`✅ Eşleşme bulundu! +${gained} Hasene`, 'success', 1500);
            
            eslestirmeSelectedCards = [];
            
            // Check if game complete
            if (eslestirmePairsFound >= eslestirmeCards.length / 2) {
                clearInterval(eslestirmeTimer);
                setTimeout(() => {
                    endGame();
                }, 2000);
            }
...
/**
 * Yazım Modu - TEST
 * Türkçe karşılığını yazma
 */
async function startYazimModuGame() {
    const data = await loadKelimeData();
    if (data.length === 0) {
        showToast('Kelime verisi yüklenemedi', 'error');
        goToMainMenu();
    ...
    document.getElementById('yazim-modu-screen').classList.remove('hidden');
    document.getElementById('yazim-total-questions').textContent = CONFIG.QUESTIONS_PER_GAME;
    
    // Focus input on Enter key
    const input = document.getElementById('yazim-input');
    input.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            document.getElementById('yazim-submit-btn').click();
        }
    });
    
    loadYazimModuQuestion();
}

function loadYazimModuQuestion() {
    if (questionIndex >= currentQuestions.length) {
        endGame();
        return;
    }
    ...
    document.getElementById('yazim-question-number').textContent = questionIndex + 1;
    document.getElementById('yazim-combo').textContent = comboCount;
    document.getElementById('yazim-session-score').textContent = formatNumber(sessionScore);
    
    // Display Arabic word
    document.getElementById('yazim-arabic').textContent = currentQuestion.kelime || currentQuestion.arabic || '';
...
function checkYazimModuAnswer() {
    const input = document.getElementById('yazim-input');
    const userAnswer = input.value.trim().toLowerCase();
    const correctAnswer = (currentQuestion.anlam || currentQuestion.translation || '').trim().toLowerCase();
    const wordId = currentQuestion.kelime_id || currentQuestion.id;
    
    // Normalize answers (remove extra spaces, handle Turkish characters)
    const normalize = (str) => str.replace(/\s+/g, ' ').trim();
    const normalizedUser = normalize(userAnswer);
    const normalizedCorrect = normalize(correctAnswer);
    
    // Check exact match or fuzzy match (allow minor typos)
    let isCorrect = normalizedUser === normalizedCorrect;
    
    // Fuzzy matching: allow 1-2 character differences for common typos
    if (!isCorrect && normalizedUser.length > 3) {
        const diff = levenshteinDistance(normalizedUser, normalizedCorrect);
        isCorrect = diff <= 1; // Allow 1 character difference
    }
    ...
    if (isCorrect) {
        ...
        showToast(`✅ Doğru! +${gained} Hasene`, 'success', 1500);
    } else {
        wrongCount++;
        comboCount = 0;
        
        input.style.borderColor = '#f44336';
        showToast(`❌ Yanlış! Doğru cevap: "${currentQuestion.anlam || currentQuestion.translation}"`, 'error', 3000);
    }
...
if (typeof window !== 'undefined') {
    window.checkHizModuAnswer = checkHizModuAnswer;
    window.handleEslestirmeCardClick = handleEslestirmeCardClick;
    window.checkYazimModuAnswer = checkYazimModuAnswer;
}
